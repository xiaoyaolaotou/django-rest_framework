...
class ServerAutoReportSerializer(serializers.Serializer):
    ...
    def validate_manufacturer(self, attrs):
        """
        验证制造商名称是否存在，不存在则创建
        :param attrs:
        :return:
        ...
        try:
            #检查制造商是否存在
            return Manufacturer.objects.get(vendor_name__exact=attrs)
        except Manufacturer.DoesNotExist:
            return self.create_manufacturer(attrs)


    def validate(self, attrs):
        print(attrs['manufacturer'],"制造商")
        print(attrs['model_name'],"产品型号")
        """
        验证制造商和产品型号是否存在
        :param attrs:
        :return:
    ...
    def check_server_network_device(self,server_obj,network):
        ...
        检查指定服务器有没有这些网卡,并做关联
        """
        network_device_queryset = server_obj.networkdevice_set.all()#取出这台服务器的网卡设备
        print(network_device_queryset,"222")
        print(network,"11111111111111111")
    ...
    def check_ip(self,network_device_obj,ifnets):
        ...
        for ips in ifnets:
            try:
                ip_obj = ip_queryset.get(ip_addr__exact=ifnets["ip_addr"])
            except IP.DoesNotExist:
                if_obj = self.create_ip(network_device_obj,ifnets)
            current_ip_queryset.append(ip_obj)
        for ip_obj in set(ip_queryset) - set(current_ip_queryset):
            ip_obj.delete()

    def create_ip(self,network_device_obj,ifnets):
        ifnets["device"] = network_device_obj
        return IP.objects.create(**ifnets)
